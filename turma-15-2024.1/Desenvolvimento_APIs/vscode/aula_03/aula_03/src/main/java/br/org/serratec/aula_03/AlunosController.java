...
public class AlunosController {
    ...
    public Aluno alterarEsp(@PathVariable int id, @RequestBody Map<String, String> object) {
        int indice = obterIndice(id);
        ...
        if (indice != -1) {
            ...
            object.forEach((key, value) -> {
                switch (key) {
                    case "nome":
                        alunos.get(indice).setNome(value);
                        break;
                    case "curso":
                        alunos.get(indice).setCurso(value);
                        break;
                    case "periodo":
                        alunos.get(indice).setPeriodo(value);
                        break;
                    default:
                        break;
                }
